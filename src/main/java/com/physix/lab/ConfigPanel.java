...
public class ConfigPanel extends JPanel implements MouseListener, MouseMotionListener {
    ...
    private void createFont() {
        try {
            File file = new File(ConfigPanel.class.getResource("/dejavu.ttf").getPath());
...
//        g.setStroke (old_stroke);
    ...
    public void paintComponent (Graphics g) {
        ...
        g2d.drawString (fps+" FPS", 600, 500 - 5);
...
//        }
    ...
    public void mouseDragged (MouseEvent e) {
        ...
        if (shapes[0].getBounds2D().getMaxX() + delta_x > 600) {
            delta_x = 600 - (int)shapes[0].getBounds2D().getMaxX();
        }
        ...
        if (shapes[0].getBounds2D().getMaxY() + delta_y > 800) {
            delta_y = 800 - (int)shapes[0].getBounds2D().getMaxY();
        }
